---
name: jobreforger-setup
description: "One-time setup for JobReforgerAI. Checks Python, installs dependencies, configures user profile (name, email, phone, resume path). Run once before using other jobreforger skills."
...
After installing Python, run /jobreforger-setup again.
```
...
If `pip` is not found, try `pip3` or `python -m pip`. Use `${CLAUDE_SKILL_DIR}/requirements-plugin.txt` if needed.
...
Use `${CLAUDE_SKILL_DIR}/requirements.txt` if needed. This takes 1-3 minutes (downloads sentence-transformers model ~80MB).
...
Setup complete! You can now use:
  ...
  /jobreforger-resume [paste job description]       — Full resume + cover letter package
  /jobreforger-tailor-resume [paste JD]             — Resume only
  /jobreforger-cover-letter [paste JD]              — Cover letter only
  /jobreforger-writing-coach [resume file]          — Improve resume writing quality
  /jobreforger-find-jobs [title] [location]         — Search and score jobs
  /jobreforger-batch-resume                         — Process multiple JDs
