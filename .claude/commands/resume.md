...
- Create output folder: `tailored-resumes/{CompanyName} - {JobTitle}/`
    ...
    result = subprocess.run(
        ['curl', '-s', '-X', 'POST', 'http://localhost:8100/score/both',
         ...
         '-d', json.dumps({'resume_path': '{base_template_path}',
                           'jd_path': '../tailored-resumes/{folder}/job_description.txt'})],
        capture_output=True, text=True, timeout=120)
    write_score_results('../tailored-resumes/{folder}', 'base_both', result.stdout)
    print('Base scores written to state.json')
except Exception as e:
    log_error('../tailored-resumes/{folder}', 'scoring_base', str(e))
    print(f'Error: {e}')
"
```
...
try:
    result = subprocess.run(
        ['curl', '-s', '-X', 'POST', 'http://localhost:8100/score/combined',
         '-H', 'Content-Type: application/json',
         '-d', json.dumps({'resume_path': '../tailored-resumes/{folder}/resume.md',
                           'jd_path': '../tailored-resumes/{folder}/job_description.txt'})],
        capture_output=True, text=True, timeout=180)
    write_score_results('../tailored-resumes/{folder}', 'tailored_combined', result.stdout)
    print('Tailored scores (ATS + HR + LLM) written to state.json')
except Exception as e:
    log_error('../tailored-resumes/{folder}', 'scoring_tailored', str(e))
    print(f'Error: {e}')
"
```
...
- Save the cover letter text to: tailored-resumes/{folder}/cover_letter.md
...
update_state('../tailored-resumes/{folder}', 'iterations', iters)
"
```
...
try:
    create_resume_from_md('../tailored-resumes/{folder}/resume.md', '../tailored-resumes/{folder}/{Name}_Resume_{Company}.docx')
    update_state('../tailored-resumes/{folder}', 'docx_resume_path', '../tailored-resumes/{folder}/{Name}_Resume_{Company}.docx')
    print('Resume DOCX created successfully')
except Exception as e:
    log_error('../tailored-resumes/{folder}', 'finalizing', f'Resume DOCX failed: {e}')
    print(f'Error: {e}')
"
```
...
from orchestration_state import update_state, log_error
try:
    with open('../tailored-resumes/{folder}/cover_letter.md', 'r') as f:
        content = f.read()
    ...
    create_ats_cover_letter(
        output_path='../tailored-resumes/{folder}/{Name}_Cover_Letter_{Company}.docx',
        name='{user_name from config.json}',
    ...
    update_state('../tailored-resumes/{folder}', 'docx_cover_letter_path', '../tailored-resumes/{folder}/{Name}_Cover_Letter_{Company}.docx')
    print('Cover Letter DOCX created successfully')
except Exception as e:
    log_error('../tailored-resumes/{folder}', 'finalizing', f'Cover Letter DOCX failed: {e}')
    print(f'Error: {e}')
"
```
...
try:
    ...
    update_state('../tailored-resumes/{folder}', 'tracker_updated', True)
    print('Tracker updated successfully')
except Exception as e:
    log_error('../tailored-resumes/{folder}', 'finalizing', f'Tracker update failed: {e}')
    print(f'Error: {e}')
"
```
...
import json
state = read_state('../tailored-resumes/{folder}')
set_phase('../tailored-resumes/{folder}', 'done')
...
3. Copy cover letter DOCX to root `cover-letters/` folder:
```
cp "tailored-resumes/{folder}/{Name}_Cover_Letter_{Company}.docx" "cover-letters/"
```
...
cleanup_state('../tailored-resumes/{folder}')
...
FOLDER: tailored-resumes/{Company} - {JobTitle}/
