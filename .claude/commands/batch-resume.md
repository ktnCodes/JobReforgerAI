...
```
Glob: tailored-resumes/**/*Resume*.docx
```
...
- Output folder: .\tailored-resumes\{Company} - {Job Title}\
...
```bash
curl -s -X POST http://localhost:8100/score/both -H "Content-Type: application/json" -d "{\"resume_path\": \"../tailored-resumes/{Company} - {Job Title}/resume.md\", \"jd_path\": \"../tailored-resumes/{Company} - {Job Title}/job_description.txt\"}"
```
