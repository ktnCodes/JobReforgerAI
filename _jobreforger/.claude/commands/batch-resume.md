...
## STEP 1: DISCOVER JDs DISCOVER JDs
...
### 3. Score Resume
Score using the CLI scorers:
```bash
python ats_scorer.py --score "applications/{Company} - {Job Title}/resume.md" "applications/{Company} - {Job Title}/job_description.txt" --json
python hr_scorer.py --score "applications/{Company} - {Job Title}/resume.md" "applications/{Company} - {Job Title}/job_description.txt" --json
```

Parse the JSON output to extract ATS total_score and HR overall_score.
