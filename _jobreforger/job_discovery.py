...
APIs:
  - JobSpy (primary): Scrapes Indeed, LinkedIn, ZipRecruiter, Glassdoor — no API keys needed
  - Remotive (secondary): Free, no auth, remote jobs only
  - Adzuna (optional): REST API, requires ADZUNA_APP_ID + ADZUNA_APP_KEY env vars
...
# ---------------------------------------------------------------------------
# JobSpy (Indeed + LinkedIn + ZipRecruiter + Glassdoor — no API keys)
# ---------------------------------------------------------------------------
...
def search_jobspy(
    ...
    """Search Indeed, LinkedIn, ZipRecruiter, Glassdoor via JobSpy (no API keys required)."""
    try:
    ...
    try:
        ...
        return _normalize_jobspy_results(df)
    except Exception:
        return []


def _normalize_jobspy_results(df) -> List[Dict[str, Any]]:
    """Convert a JobSpy DataFrame to the common job dict schema."""
    results = []
    for _, row in df.iterrows():
        salary_min = None
        salary_max = None
        try:
            salary_min = int(row.get("min_amount") or 0) or None
            salary_max = int(row.get("max_amount") or 0) or None
...
def discover_jobs(
    ...
    # --- Step 1: Search APIs ---
    # Priority: JobSpy (no keys) → Remotive (no keys, remote only) → Adzuna (optional, requires keys)
    has_adzuna = _adzuna_configured()
    seen_ids: set = set()

    for query in search_queries:
        # JobSpy: searches Indeed, LinkedIn, ZipRecruiter, Glassdoor — no API keys needed
        for job in search_jobspy(query, location=location or "remote"):
            if job["id"] not in seen_ids:
                seen_ids.add(job["id"])
                all_jobs.append(job)

        # Remotive: free, public API — remote jobs only
        for job in search_remotive(query):
            if job["id"] not in seen_ids:
                seen_ids.add(job["id"])
                all_jobs.append(job)

        # Adzuna: optional, only if API keys are configured in .env
        if has_adzuna:
    ...
    if ai_analysis and os.getenv("ANTHROPIC_API_KEY", ""):  # type: ignore[arg-type]
