...
State file lives inside each application folder:
    ../tailored-resumes/Takeda - Associate Medical Director/state.json
...
CLI usage (debugging):
    ...
    python orchestration_state.py --read "../tailored-resumes/Takeda - Associate Medical Director"
"""
...
def init_state(
    ...
    ----------
    folder : str
        Application output folder (e.g. ``../tailored-resumes/Takeda - Associate Medical Director``).
    company : str
        Company name.
    job_title : str
        Job title / position.
...
def check_phase_health(
    ...
    """Check whether background agents completed their work for a given phase.

    This is a non-blocking health check (unlike ``wait_for_keys`` which blocks).
    It reads state.json once and returns a diagnostic dict indicating which
    expected keys are present, which are missing, and whether any errors were
    logged for the current or specified phase.
    ...
    expected_keys : list[str], optional
        Top-level keys that should be present if agents completed successfully.
        For example, after base scoring: ``["base_scores"]``.
    timeout : int
        Not used for blocking — included for future retry logic.  Currently
        stored in the returned dict so callers can decide whether to retry.
    phase_name : str
        Phase name to filter errors for (e.g. ``"scoring_base"``).  If empty,
        all errors are returned.

    Returns
    -------
    dict
        ``healthy`` (bool): True if all expected keys are present and no
        errors were logged for this phase.

        ``present_keys`` (list): Which expected keys were found.

        ``missing_keys`` (list): Which expected keys were NOT found.

        ``phase_errors`` (list): Error entries matching *phase_name*.

        ``current_phase`` (str): The ``meta.phase`` value.
    ...
    if not state:
        return {
            ...
            "suggestion": "No state.json found — workflow may not have started.",
        }

    current_phase = state.get("meta", {}).get("phase", "unknown")
    expected_keys = expected_keys or []

    present = [k for k in expected_keys if k in state]
    missing = [k for k in expected_keys if k not in state]

    all_errors = state.get("errors", [])
    if phase_name:
        phase_errors = [e for e in all_errors if e.get("phase", "") == phase_name]
    else:
        phase_errors = all_errors

    healthy = len(missing) == 0 and len(phase_errors) == 0

    # Build suggestion
    if healthy:
        suggestion = "All background agents completed successfully."
    elif missing and phase_errors:
        suggestion = (
            f"Missing keys: {missing}. Errors logged: {len(phase_errors)}. "
            "Consider falling back to direct (non-background) scoring."
        )
    elif missing:
        suggestion = (
            f"Missing keys: {missing}. Agents may still be running or failed silently. "
            "Try wait_for_keys() with a short timeout, or fall back to direct scoring."
        )
    else:
        suggestion = (
            f"{len(phase_errors)} error(s) logged for phase '{phase_name}'. "
            "Review errors and consider retrying the failed agent."
        )
...
def safe_wait_for_keys(
    folder: str,
    keys: list,
    timeout: int = 60,
    poll_interval: float = 2.0,
) -> tuple:
    """Like ``wait_for_keys`` but returns (state, error) instead of raising.
    ...
    -------
    tuple[dict, str | None]
        ``(state, None)`` on success — all keys found within timeout.
        ``(partial_state, error_message)`` on timeout — includes what was found.
    """
    try:
        state = wait_for_keys(folder, keys, timeout=timeout, poll_interval=poll_interval)
        return state, None
