#!/usr/bin/env python3
# ⚠️ DEPRECATED: This standalone CLI tool has been superseded by Claude Code
# slash commands (/resume, /tailor-resume, /cover-letter). It requires an
# Anthropic API key which the modern workflow does not need.
# Kept for reference. Use the slash commands instead.
"""
Resume Builder CLI Tool (DEPRECATED)
-------------------------------------
This standalone CLI has been replaced by Claude Code slash commands.
See the README for the recommended workflow.

Legacy usage (requires ANTHROPIC_API_KEY):
    python resume_builder.py

Recommended usage (no API key needed):
...
def create_output_folder(company_name: str, config: dict) -> Path:
    ...
    base_dir = Path(__file__).parent / config.get("output_base_dir", "../tailored-resumes")
