...
def calculate_hr_score(
    ...
    # 2. KNOCKOUT CHECK — only applies when experience was actually parsed (> 0)
    # If total_years_experience == 0, parsing likely failed (empty/unparseable resume) — skip knockout
    if candidate.total_years_experience > 0 and candidate.total_years_experience < 0.3 * jd.required_years:
...
def calculate_hr_score_from_text(resume_text: str, jd_text: str) -> "HRScoreResult":
    ...
    # Only knockout when experience was actually parsed — skip if 0 (unparseable/empty resume)
    if candidate.total_years_experience > 0 and candidate.total_years_experience < 0.3 * jd.required_years:
        return HRScoreResult(
            overall_score=0,
            recommendation="AUTO-REJECT",
