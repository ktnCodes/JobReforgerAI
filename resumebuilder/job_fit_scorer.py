...
TITLE_TO_EXPERIENCE_TYPE = {
    ...
    'embedded_software': [
        r'\bembedded\b', r'\bfirmware\b', r'\bbsp\b', r'\bdevice\s+driver\b',
        r'\brtos\b', r'\bmicrocontroller\b', r'\bmcu\b',
    ],
    'software_engineering': [
        r'\bsoftware\s+(?:engineer|developer|architect)\b',
        r'\bprogrammer\b', r'\bfull.?stack\b', r'\bbackend\b', r'\bfrontend\b',
    ],
    'controls_automation': [
        r'\bcontrols?\s+(?:engineer|system)\b', r'\bautomation\b',
        r'\bplc\b', r'\bscada\b', r'\brobotics\b',
    ],
    'systems_engineering': [
        r'\bsystems?\s+engineer\b', r'\bsystem\s+architect\b',
...
def _suggest_alternative_titles(profile: EnrichedProfile, req: ExtractedRequirements) -> List[str]:
    """Suggest better-fitting job titles based on profile and detected domain."""
    suggestions = []
    domain = getattr(req, 'domain', 'general')

    # --- Embedded / Agriculture / Technology ---
    has_embedded = profile.years_by_type.get('embedded_software', 0) > 0
    has_sw_eng = profile.years_by_type.get('software_engineering', 0) > 0
    has_controls = profile.years_by_type.get('controls_automation', 0) > 0
    has_systems = profile.years_by_type.get('systems_engineering', 0) > 0
    has_leadership = profile.years_by_type.get('leadership', 0) > 0

    if domain in ('embedded', 'technology') or has_embedded or has_sw_eng:
        if has_embedded:
            suggestions.extend([
        ...
        if has_leadership and (has_embedded or has_sw_eng):
            suggestions.extend([
                'Engineering Manager',
                'Technical Lead',
    ...
    # --- Clinical / Pharma / Healthcare (original logic) ---
    ...
    if domain in ('clinical_research', 'pharma_biotech', 'healthcare') or has_md or has_research:
        if has_md:
            suggestions.extend([
                'Medical Monitor',
